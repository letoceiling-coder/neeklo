...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (!Schema::hasColumn('support_tickets', 'external_id')) {
            Schema::table('support_tickets', function (Blueprint $table) use ($isSqlite) {
                if ($isSqlite) {
        ...
        // Переименовываем theme в subject (только для MySQL/MariaDB)
        if (!$isSqlite && Schema::hasColumn('support_tickets', 'theme')) {
            try {
        ...
        if (!Schema::hasColumn('support_messages', 'external_message_id')) {
            Schema::table('support_messages', function (Blueprint $table) use ($isSqlite) {
                if ($isSqlite) {
        ...
        // Переименовываем message в body (только для MySQL/MariaDB)
        if (!$isSqlite && Schema::hasColumn('support_messages', 'message') && !Schema::hasColumn('support_messages', 'body')) {
            try {
        ...
        // Изменяем enum sender: local|crm → tma|crm (только для MySQL/MariaDB)
        if (!$isSqlite) {
            try {
